# config.py

# Pygame Screen Dimensions
SCREEN_WIDTH = 1500
SCREEN_HEIGHT = 600

# Frame per second limit
FPS_LIMIT = 60

# Player details
PLAYER_1 = 1
PLAYER_2 = 2
PLAYER_NUMBERS = [PLAYER_1, PLAYER_2]

# Game board
INIT_SEEDS = 6
INIT_HOUSE_ROW = INIT_SEEDS # 6
TOTAL_HOUSE = INIT_HOUSE_ROW * 2 # 12
MAX_HOUSE_COUNT = (INIT_SEEDS + 1) * 2 # 14
MAX_INDEX = MAX_HOUSE_COUNT - 1 # 13
BOARD_HOUSES = [INIT_SEEDS] * INIT_HOUSE_ROW + [0] + [INIT_SEEDS] * INIT_HOUSE_ROW + [0]

# Game houses
PLAYER_1_STORE = MAX_HOUSE_COUNT - 1 # 13
PLAYER_2_STORE = INIT_HOUSE_ROW # 6
PLAYER_1_MIN_HOUSE = PLAYER_2_STORE + 1 # 7
PLAYER_1_MAX_HOUSE = PLAYER_1_STORE - 1 # 12
PLAYER_2_MIN_HOUSE = 0 # 0
PLAYER_2_MAX_HOUSE = PLAYER_2_STORE - 1 # 5
STORE_INDICES = [PLAYER_1_STORE, PLAYER_2_STORE]

# Animation
ANIMATION_SPEED = 10
SLEEP_TIME = 0.1
CAPTURE_PHASES = [1, 2]

# Game colors
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
RED = (255, 0, 0)

# Seed font and size
SEED_FONT = None
SEED_FONT_SIZE = 36
SEED_COLOR = WHITE
STORE_SEED_FONT = None
STORE_SEED_FONT_SIZE = 60
HOUSE_INDEX_FONT = None
HOUSE_INDEX_FONT_SIZE = 24
HOUSE_INDEX_COLOR = RED

# cursor
CURSOR_IMAGE = "../assets/handcursor.png"
CURSOR_FONT = None
CURSOR_FONT_SIZE = 36
CURSOR_FONT_COLOR = BLACK

# pause message font and size
PAUSE_FONT = None
PAUSE_FONT_SIZE = 50
PAUSE_MSG_COLOR = RED

# capture message font and size
CAPTURE_FONT = None
CAPTURE_FONT_SIZE = 50
CAPTURE_MSG_COLOR = RED

# pause button 
PAUSE_BUTTON_FONT = None
PAUSE_BUTTON_FONT_SIZE = 36
PAUSE_BUTTON_DIM = (10, 70, 100, 50)
PAUSE_BUTTON_COLOR = BLACK

# Restart button
RESTART_BUTTON_FONT = None
RESTART_BUTTON_FONT_SIZE = 36
RESTART_BUTTON_DIM = (10, 10, 100, 50)
RESTART_BUTTON_COLOR = BLACK

# House sizes
STORE_SIZE = 90
HOUSE_SIZE = 45

# Colors and messages
SCREEN_FILL_COLOR = WHITE

# turn message font and size
TURN_MSG_FONT = None
TURN_MSG_FONT_SIZE = 36
TURN_MSG_COLOR = BLACK
