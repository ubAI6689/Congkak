...
class CongkakGame:
    ...
    def update(self):
        ...
        elif self.animating and not self.pause:
            self.animate_seeds_movement()
        else:
            if not self.pause:
    ...
    def end_move(self):
        ...
        if not self.animating and self.board.check_game_end():
    ...
    def handle_capture_state(self):
        ...
        if not self.board.is_players_house(self.current_player.number, self.target_house):
            self.end_move()
            return

        if not self.passed_store:
            self.end_move()
            return
