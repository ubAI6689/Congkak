...
class Animator:
    ...
    #             if target_house != self.game.get_opponent(self.game.current_player).store:

    #                 if player_number == 1:
    #                     # Decrement the number of seeds to move
    #                     self.set_seeds_to_move_1(seeds_to_move - 1)
    #                     # Update the target house
    #                     self.set_target_house_1((target_house + 1) % MAX_HOUSE_COUNT)
    #                     # Update the target position
    #                     self.set_target_pos_1(self.game.get_pos_of_house(target_house))
    #                     # Update the cursor position to the target position
    #                     # set_cursor_pos(self.get_target_pos_1())

    #                 elif player_number == 2:
    #                     # Decrement the number of seeds to move
    #                     self.set_seeds_to_move_2(seeds_to_move - 1)
    #                     # Update the target house
    #                     self.set_target_house_2((target_house + 1) % MAX_HOUSE_COUNT)
    #                     # Update the target position
    #                     self.set_target_pos_2(self.game.get_pos_of_house(target_house))
    #                     # Update the cursor position to the target position
    #                     # set_cursor_pos(self.get_target_pos_2())
